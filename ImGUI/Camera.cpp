...
bool Camera::Initialize(HINSTANCE hInstance, HWND hwnd, Input*input, int width, int height, XMFLOAT3 eye , XMFLOAT3 at)
{
	bool hr = true;
	//m_input = input;
	hr = m_input->Initialize(hInstance, hwnd, width, height);
	...
	XMVECTOR v_eye = XMLoadFloat3(&Eye);
	XMVECTOR v_at = XMLoadFloat3(&At);
	XMVECTOR v_up = XMLoadFloat3(&Up);

	this->c_view = XMMatrixLookAtLH(v_eye, v_at, v_up);
	...
	//Initialize Projection Matrix
	projection = XMMatrixPerspectiveFovLH(XM_PIDIV2, width / (FLOAT)height, 0.01f, 100.0f);
	ortho = XMMatrixOrthographicLH(width, height, 0.01f, 100.0f);
...
void Camera::Frame()
{
	...
	else if(m_input->isMouseLeftDown())  //平移变换
	{
...
XMMATRIX Camera::GetViewConstMatrix()
{
	return this->c_view;
}
